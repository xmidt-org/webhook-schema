...
func TestLevelUnmarshalling(t *testing.T) {
	tests := []struct {
		...
		{
			description: "Nonexistent level failure",
			config:      []byte("FOOBAR"),
			invalid:     true,
		},
	}
...
func TestLevelState(t *testing.T) {
	tests := []struct {
		...
		{
			description: "lastLevel invalid",
			val:         lastLevel,
			expectedVal: "unknown",
			invalid:     true,
		},
		{
			description: "Nonexistent positive LevelType invalid",
			val:         lastLevel + 1,
			expectedVal: "unknown",
			invalid:     true,
		},
		{
			description: "Nonexistent negative LevelType invalid",
			val:         UnknownLevel - 1,
			expectedVal: "unknown",
			invalid:     true,
		},
